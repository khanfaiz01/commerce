...
import reservedInvCheckOut from '@salesforce/apex/B2BCartControllerSample.reservedInvCheckOut';
import reservedInvClearCart from '@salesforce/apex/B2BCartControllerSample.reservedInvClearCart';
import getSKU from '@salesforce/apex/B2BCartControllerSample.getSKU';
import getInventroyData from '@salesforce/apex/B2BGetInfo.getInventroyData';
import { ShowToastEvent } from 'lightning/platformShowToastEvent';
import getcartitemId from '@salesforce/apex/B2BCartControllerSample.getcartitemId';
...
export default class CartContents extends NavigationMixin(LightningElement) {
    ...
    handleQuantityChanged(evt) {
        ...
        getcartitemId({
         ...
         .then((result)=>{
           getInventroyData({productId:result.productId})
           .then((result1)=>{
            if (quantity > result1.Available_For_Purchase__c) {
                // Show an error message to the user
                this.dispatchEvent(
                  new ShowToastEvent({
                    title: 'Error',
                    message: 'There are only ' + result1.Available_For_Purchase__c + ' items available. Please enter a valid quantity.',
                    variant: 'error',
                    mode: 'dismissable'
                  })
                );
