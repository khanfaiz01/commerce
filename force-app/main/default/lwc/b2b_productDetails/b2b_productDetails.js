...
import reservedInventory from '@salesforce/apex/B2BGetInfo.reservedInventory';
    ...
    addToCart(event) {
        if (event.detail.quantity > this.data.Available_For_Purchase__c) {
            ...
            this.dispatchEvent(
              new ShowToastEvent({
                title: 'Error',
                message: 'There are only ' + this.data.Available_For_Purchase__c + ' items available. Please enter a valid quantity.',
                variant: 'error',
                mode: 'dismissable'
              })
            );
        ...
        addToCart({
            ...
            .then(() => {
                ...
                reservedInventory(
                    {sku:this.displayableProduct.sku,
                        reservedQuantity:event.detail.quantity,
                        productId:this.recordId
                    });
