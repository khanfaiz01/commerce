...
export default class B2b_productDetailsDisplay extends NavigationMixin(
    ...
    desc = 'please notify when the product will be avaliable inStock';
    ...
    // A bit of coordination logic so that we can resolve product URLs after 
    //the component is connected to the DOM,
    ...
    /**
     * Gets or sets the ordered hierarchy of categories to which the product belongs, 
    ...
    /**
     * Updates the breadcrumb path for the product, 
     * resolving the categories to URLs for use as breadcrumbs.
     *
    ...
    createProductCase(){
        insertCase({descr:this.desc, userId:this.UserId})
        .then(result => {
            this.message = result;
            ...
            if(this.message !== undefined) {
                ...
                this.dispatchEvent(
                    new ShowToastEvent({
                        title: 'Success',
                        message: 'Thank you!, you will be notified when product is inStock',
                        variant: 'success',
                    }),
                );
            }
